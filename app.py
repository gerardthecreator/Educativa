...
# En Render, debes configurarla como una Variable de Entorno.
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'panita-ciencia-secret-key-local-dev')


# --- FUNCIÓN AUXILIAR PARA CARGAR CONTENIDO DE LECCIONES ---
def load_content():
    content_dir = 'content'
    subjects = {}
    if not os.path.exists(content_dir):
        print(f"Advertencia: La carpeta '{content_dir}' no existe.")
        return {}
                ...
                if lesson_file.endswith('.json'):
                    ...
                    try:
                        with open(lesson_path, 'r', encoding='utf-8') as f:
                            ...
                            # El 'slug' es el identificador único de la lección en la URL.
                            lesson_data['slug'] = os.path.splitext(lesson_file)[0]
                            subjects[subject_name].append(lesson_data)
                    except Exception as e:
                        print(f"Error cargando la lección {lesson_file}: {e}")
...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Hubo un error al registrar el usuario. Inténtelo más tarde.', 'error')
        finally:
            if conn:
                conn.close()
...
def logout():
    if 'user_id' in session:
        ...
        conn = db.get_db_connection()
        # Limpia el token para permitir un nuevo inicio de sesión.
...
@app.route('/')
def index():
    # La página raíz redirige al login o al dashboard.
    if 'user_id' in session:
        return redirect(url_for('dashboard'))
    return redirect(url_for('login'))
...
def dashboard():
    ...
    # Convierte las notas a un diccionario para un acceso más fácil en la plantilla HTML.
...
def take_quiz(subject_slug, lesson_slug):
    ...
    if request.method == 'POST':
        ...
        # Calificar las respuestas del usuario.
